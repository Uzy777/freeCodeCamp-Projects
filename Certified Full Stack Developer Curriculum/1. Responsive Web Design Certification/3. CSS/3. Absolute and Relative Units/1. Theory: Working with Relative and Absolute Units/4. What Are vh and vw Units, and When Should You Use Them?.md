...
---
## Questions
*Q1) What does 50vh represent in CSS?*

A) 50 pixels.  
B) Half the height of the viewport.  
C) 50% of the element's parent height.  
D) Half the width of the viewport.  
...
*Q2) Which of the following is a valid use case for vw units?*

A) Setting a fixed width for a sidebar.  
B) Defining the maximum width of an image.  
C) Creating a square element that's always 20% of the viewport width.  
D) Setting the line-height of paragraph text.  
...
*Q3) What potential issue might arise from using vh units for critical layout elements on mobile devices?*

A) They're not supported on older mobile browsers.  
B) They can cause layout shifts when the mobile browser's address bar appears or disappears.  
C) They make the website load slower on mobile devices.  
D) They don't work with touch screens.  
