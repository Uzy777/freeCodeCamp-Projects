...
---
## Questions
*Q1) What is the difference between the :first-of-type and :last-of-type pseudo-classes?*

A) :first-of-type targets the first element of a specific type within its parent, while :last-of-type targets the last element of a different type.  
B) :first-of-type and :last-of-type both target the first occurrence of an element but in different sections of the document.  
C) :first-of-type selects the first occurrence of a specific element type within its parent, while :last-of-type selects the last occurrence of that same element type within its parent.  
D) :last-of-type applies styles to the first and last elements within the document, while :last-of-type applies styles to all elements of a specific type.  
...
*Q2) What is the difference between the :first-child and :last-child pseudo-classes?*

A) :first-child targets the first element within its parent, while :last-child targets the last element within a different parent.  
B) :first-child targets the first element within its parent, while :last-child targets the last element within the same parent.  
C) :first-child targets the first element of a specific type within its parent, while :last-child targets the last element of a different type within its parent.  
D) :first-child targets the first and last elements within a parent, while :last-child targets all other elements.  
...
*Q3) Which pseudo-class allows you to target elements that have no children, including those that contain only whitespace?*
