...
A) The switch statement will throw an error.  
B) The program will exit the switch block immediately.  
C) The code will continue to the next case, regardless of whether it matches.  
D) Nothing, break is optional in switch statements.  
...
*Q2) Which of the following is a key advantage of using a switch statement over an if/else chain?*

A) switch statements can handle more complex conditions.  
B) switch statements can compare multiple variables.  
C) switch statements are typically more concise for comparing a single variable against many values.  
D) switch statements always perform faster than if/else chains.  
...
*Q3) In JavaScript, what type of comparison does a switch statement use?*
