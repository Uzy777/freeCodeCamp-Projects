...
---
## Questions
*Q) Which of these actions permanently removes a file or folder from your computer?*

A) Moving the file to a different folder.  
B) Renaming the file or folder.  
C) Emptying the Recycle Bin or Trash.  
D) Copying the file or folder to an external drive.  
...
*Q) What is the primary difference between copying and moving a file?*

A) Copying creates a duplicate of the file, while moving transfers the original file to a new location.  
B) Moving creates a duplicate of the file in a new location, while copying transfers the original file.  
C) There is no difference between copying and moving a file.  
D) Copying deletes the original file, while moving creates a new file.  
...
*Q) Why are folders important for organizing your computer?*

A) Folders make files more difficult to find.  
B) Folders are only necessary for storing large files.  
C) Folders help you categorize and find files efficiently.  
D) Folders slow down your computer if you have too many.  
