...
*Q1) Which notation would you use to access a property of an object if the property name contains spaces?*

A) Dot notation  
B) Bracket notation  
C) Both dot and bracket notation  
D) Neither, it's not possible to have property names with spaces  
...
*Q2) What's an advantage of using bracket notation over dot notation when accessing object properties?*

A) It's faster to type.  
B) It allows you to use variables as property names.  
C) It's the only way to access nested properties.  
D) It automatically converts property names to uppercase.  
...
*Q3) Why is it said that most things in JavaScript are objects?*

A) Because everything in JavaScript has properties and methods.  
B) Because JavaScript was originally designed for object-oriented programming.  
C) Because objects are the only data type in JavaScript.  
D) Because even primitive data types can be treated as objects in certain contexts.  
